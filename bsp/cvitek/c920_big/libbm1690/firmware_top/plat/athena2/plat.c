...
{
	u32 index = shm_read32(SHARE_REG_C906_FW_LOG_WP);
	...
	shm_write32(SHARE_REG_C906_FW_LOG_WP, index);
}
...
void setup_plic(void)
{
	...
	shm_write32(SHARE_REG_FW_STATUS, C906_START_STEP_UNMASK_ALL_INTC);
}
...
int athena2_plat_init(void)
{
#if 1
	work_mode = WORK_MODE_SOC;
#else
	work_mode = shm_read32(SHARE_REG_C906_FW_MODE);
#endif
