#include "nodechip_sdma_are_test_utils.h"

void nodechip_sdma_lossy_decompress_are_test(
    ...
    tpu_sdma_cpy_S2S(l2_reduce, reduce, &shape, NULL, NULL, DT_FP32);
    tpu_sdma_cpy_S2S(l2_reduce_buffer, input, &shape, NULL, NULL, DT_FP32);
    // test gdma tensor reduce global to l2
    // TEST_SDMA_TENSOR_REDUCE(S);
    // test gdma tensor reduce l2 to l2
    // TEST_SDMA_TENSOR_REDUCE(L2);
    ...
    tpu_sdma_cpy_S2S(output, l2_reduce, &shape, NULL, NULL, DT_FP32);
    tpu_poll();
}